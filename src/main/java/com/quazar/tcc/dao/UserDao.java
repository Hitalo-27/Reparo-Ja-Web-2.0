...
public class UserDao extends ConexaoBd{
	...
	public void cadastrarUser(User user) {
		String create = "INSERT INTO tb_users(nome, email, senha, cpf, cep, rua, numeroCasa, bairro, id_tipoUser)"
				+ "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			...
			stmt.setInt(7, user.getNumeroCasa());
			stmt.setString(8, user.getBairro());
			stmt.setLong(9, user.getTipoUser().getId());
			stmt.executeUpdate();
			con.close();
		} catch (Exception e) {
			e.printStackTrace();
        ...
        try{
              ...
              while(rs.next()){
            	  User user = new User(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), 
            			  rs.getString(6), rs.getString(7), rs.getInt(8), rs.getString(9), rs.getLong(10));
            	  users.add(user);
              }
              con.close();
              return users;
	...
	public void alterarUser(User user) {
		String update = "UPDATE tb_users SET nome = ?, email = ?, senha = ?, cpf = ?, cep = ?, rua = ?, numeroCasa = ?, bairro = ? WHERE id = ?";
		try {
			Connection con = conectar();
